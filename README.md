...
│   │   ├── session.ts         # Per-channel SDK session, streaming-input queue, relay, idle reap, orphan sweep
│   │   ├── policy.ts          # canUseTool gate: protected paths + denied shell commands
│   │   ├── bridge.ts          # In-process MCP server re-exporting Discord/memory tools (no evolve_*)
