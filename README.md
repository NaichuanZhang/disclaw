...
| `PILOT_ANTHROPIC_API_KEY` | No | Opt into API-key auth for pilot sessions. Unset, pilot uses `~/.claude` subscription credentials and the bot's own `ANTHROPIC_API_KEY` is never forwarded to the child. |
...
│   │   ├── messages.ts        # Message pipeline: filter → session → lock → voice transcribe → pilot route → agent → thread reply
...
│   ├── pilot/                 # Pilot mode: Claude Agent SDK sessions per channel
