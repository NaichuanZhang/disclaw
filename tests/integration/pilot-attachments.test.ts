// ---------------------------------------------------------------------------
// Pilot mode — attachment inbox
//
// Pilot can't take Claude content blocks, so a Discord attachment is downloaded
...
import {
  ...
  prunePilotInbox,
} from "../../src/pilot/attachments.js";
import { PILOT_INBOX_DIR } from "../../src/pilot/attachments.js";
import { PILOT_WORKSPACE_DIR } from "../../src/pilot/session.js";

let dir: string;

beforeEach(() => {
  dir = mkdtempSync(path.join(tmpdir(), "pilot-inbox-"));
});
...
describe("PILOT_INBOX_DIR", () => {
  it("sits inside the pilot workspace", () => {
    ...
    expect(PILOT_INBOX_DIR).toBe(path.join(PILOT_WORKSPACE_DIR, "inbox"));
  });
});
...
describe("prunePilotInbox", () => {
  it("removes directories older than the window and keeps recent ones", () => {
    ...
    const removed = prunePilotInbox({ inboxDir: dir, maxAgeMs: 24 * 60 * 60 * 1000 });
  ...
  it("is a no-op when the inbox does not exist", () => {
    expect(prunePilotInbox({ inboxDir: path.join(dir, "nope") })).toBe(0);
  });
});
...
describe("pilot message dispatch", () => {
  ...
  it("no longer refuses attachments in a pilot channel", () => {
    expect(messagesSrc).not.toContain("Pilot mode doesn't handle image/document attachments");
    expect(messagesSrc).toContain("savePilotAttachments(");
    expect(messagesSrc).toContain("formatAttachmentBlock(");
  });

  it("still bails when a pilot message has neither text nor attachments", () => {
    expect(messagesSrc).toContain(
      "inPilotChannel && !cleanContent && message.attachments.size === 0",
    );
  });

  it("logs the user side of a pilot turn into the conversation tables", () => {
    expect(messagesSrc).toContain('role: "user"');
    expect(messagesSrc).toContain("pilotLogSessionId");
    expect(messagesSrc).toContain("logSessionId: pilotLogSessionId");
  });
