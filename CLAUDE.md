...
Enabling/disabling pilot mode is a data change (a `channel_configs.settings` flag), not a code change. Remaining gaps: pilot sessions keep their own conversation state, so `/clear` and `/compact` don't reach them; there is no message-edit streaming; and a cron `agentTurn` job pointed at a pilot channel still runs through the main agent.
