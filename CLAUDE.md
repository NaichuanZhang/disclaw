...
- `policy.ts` — **not enforced today.** Pilot runs with `permissionMode: 'bypassPermissions'` + `allowDangerouslySkipPermissions: true`, so `canUseTool` never fires and pilot tool calls are unguarded (they can reach `src/**`, `.env`, `git push`). The pure rule functions are kept and unit-tested so the gate can be re-wired (restore `canUseTool`, or move it into a `PreToolUse` hook, which still fires in bypass mode). The rules block `src/**`, `.env`, `.git/**`, `worktrees/**`, `migrations/`, `start.sh`, `~/.ssh`, `~/.config/gh`, `~/.claude/.credentials.json`, plus `git push`, `gh pr|auth|api`, `sudo`, package publishing and pipe-to-shell. Pure functions (`checkPilotPath`, `checkPilotCommand`, `evaluatePilotToolCall`) are unit-tested in `tests/integration/pilot-guards.test.ts`.
...
- Prompt parity: `session.ts#buildIdentityPrompt()` prepends `getSoul()` and `MEMORY_RECALL_INSTRUCTIONS` (from `shared/prompt-fragments.ts`) and `buildCavemanPrompt()` appends `buildCavemanInstructions(level)` when the channel has `settings.cavemanLevel`, so a pilot channel keeps the bot's identity, its memory habits and `/caveman`. Both fragments are *the same objects* the main agent uses — they were duplicated strings in `agent/agent.ts` and drifted. The caveman level is read once, when the SDK child starts: a level changed mid-session applies from the next session (idle reap or `/stop`). Turn failures call `recordSignal({type:"error", source:"pilot"})` so the reflection daemon sees pilot errors like agent errors.
- `cron-route.ts` — the pure half of "where should a cron `agentTurn` run": `planPilotCronRoute` returns the channel whose flag decides (a thread's parent), the channel the session keys to, and whether the caller must create a thread first. The DB read (`isPilotChannelId`) and the Discord calls stay in `index.ts`, which is what makes the rules testable.
...
- `env.ts` — the child inherits an explicit allowlist, not `process.env`, so `DISCORD_TOKEN`/`GH_TOKEN`/`ANTHROPIC_API_KEY` can't be read out with one `Bash` call.

Enabling/disabling pilot mode is still just a `channel_configs.settings` flag, but `/pilot on|off|status` writes it for you (resolving a thread to its parent, and stopping any live session on `off`) instead of requiring a hand-edited row. `/clear` in a pilot channel is routed to `resetPilotSession` — our conversation rows are not what the SDK session reads, so clearing them looked like a no-op. There is no `/compact` command in this codebase at all. A cron `agentTurn` job now runs on the runtime its channel is configured for: `cron/service.ts` hands the callback a `CronAgentTurnContext` (delivery channel + job identity), `runCronAgentTurn` in `index.ts` plans the route with `pilot/cron-route.ts` (pure: thread→parent inheritance read from the client *cache*, so there is no extra fetch, and no nested thread when the target already is one), and a pilot-flagged channel gets `ensureThread` + `submitToPilotSession` instead of `processAgentTurn`. Submission is fire-and-forget, so the run record only says where it went, the cron per-job timeout no longer bounds the work (the turn watchdog does), and a per-job `model` override does not apply — it is logged as ignored. Any failure in that path falls back to the main agent so a scheduled job never silently stops running. Remaining gap: no message-edit streaming.
