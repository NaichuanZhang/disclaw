...
`src/pilot/` is a **second runtime**, not a refactor of the first. A channel with `settings.pilot = true` in `channel_configs` is routed in `bot/messages.ts` (step 6e, after voice transcription, before thread creation) to a Claude Agent SDK session instead of `agent/agent.ts`. The normal path is untouched for every other channel.

- `session.ts` — one `query()` session per channel. The prompt is an async generator, so `submit()` can inject a new Discord message into a turn that is already running. Relays assistant text and tool-call lines to the channel (buffered per message; no message-edit streaming). Persists the SDK session id in `channel_configs.settings.pilotSessionId` and passes it as `resume` after a restart. Idle-reaped (`PILOT_IDLE_MS`), stopped by `/stop` (aborting the child kills its shell descendants), and orphaned children are swept at boot by matching `/proc/<pid>/cwd` against the pilot workspace.
